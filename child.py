  ...
  def _forward(self, x, y, model_params, init_states, is_training=False):
    ...
    if is_training:
      emb = tf.layers.dropout(
          emb, self.params.drop_i, [batch_size, 1, hidden_size], training=True)  # 控制每个时间步drop掉同样的
    ...
    top_s = all_s  # [batch_size, num_steps, hidden_size]
    if is_training:
      top_s = tf.layers.dropout(
          top_s, self.params.drop_o,
          [self.params.batch_size, 1, self.params.hidden_size], training=True)  # 添加noise mask，确保时间步上的drop一致。

    carry_on = [tf.assign(prev_s, out_s)]
    logits = tf.einsum('bnh,vh->bnv', top_s, w_soft)  # [batch_size, num_steps, vocabe_size]
    ...
    return reg_loss, loss  # loss是纯损失，reg loss里有l2正则损失。
